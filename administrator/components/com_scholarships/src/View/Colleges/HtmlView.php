...
namespace OURF\Component\Scholarships\Administrator\View\Colleges;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar()
    {
        ...
        ToolbarHelper::title(Text::_('COM_SCHOLARSHIPS_MANAGER_COLLEGES'), 'address scholarship');

        if ($canDo->get('core.create') || \count($user->getAuthorisedCategories('com_scholarships', 'core.create')) > 0)
        {
            $toolbar->addNew('colleges.add');
        }
        ...
        if (!$this->isEmptyState && ($canDo->get('core.edit.state') || \count($this->transitions)))
        {
            ...
            if (\count($this->transitions))
            {
                ...
                $cmd = "Joomla.submitbutton('colleges.runTransition');";
            ...
            {
                $childBar->publish('colleges.publish')->listCheck(true);

                $childBar->unpublish('colleges.unpublish')->listCheck(true);

                $childBar->standardButton('featured')
                    ->text('JFEATURE')
                    ->task('colleges.featured')
                    ->listCheck(true);

                $childBar->standardButton('unfeatured')
                    ->text('JUNFEATURE')
                    ->task('colleges.unfeatured')
                    ->listCheck(true);

                $childBar->archive('colleges.archive')->listCheck(true);

                $childBar->checkin('colleges.checkin')->listCheck(true);

                if ($this->state->get('filter.published') != ScholarshipsComponent::CONDITION_TRASHED)
                {
                    $childBar->trash('colleges.trash')->listCheck(true);
                }
            }
        ...
        {
            $toolbar->delete('colleges.delete')
        ...
        $toolbar->help('ScholarshipsCollege');
    }
}